...
The protocol currently supports two actions:

- `put`: create or update a file.
- `delete`: tombstone a file.

`rename` is planned but not currently modeled as a first-class Worker protocol action.
...
Uploads a raw chunk body. The Worker computes SHA-256 over the body and rejects uploads where the computed hash does not match `:hash`.
...
Downloads a raw chunk body from R2.
...
For `put`, all chunks must already be registered. If a chunk is missing, the Durable Object returns:
...
Returns changes after global version `N`.
